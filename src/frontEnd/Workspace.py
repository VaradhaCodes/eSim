...
class Workspace(QtWidgets.QDialog):
    ...
    def init_ui(self):
        ...
        button_box.addButton(self.default_btn, QtWidgets.QDialogButtonBox.ButtonRole.ActionRole)
        button_box.addButton(self.ok_btn, QtWidgets.QDialogButtonBox.ButtonRole.AcceptRole)
    ...
    def _refresh_project_explorer(self):
        """Refresh the project explorer tree to match the saved state.

        Called immediately after the dialog is accepted so the Project
        Explorer reflects the workspace the user just chose.
        """
        view = self._app_view
        if view is None:
            return
        try:
            tree = view.obj_Mainview.obj_projectExplorer.treewidget
            tree.clear()
            for parent, children in self.obj_appconfig.project_explorer.items():
                view.obj_Mainview.obj_projectExplorer.addTreeNode(
                    parent, children
                )
        except Exception as e:
            # Project explorer refresh must never prevent the main
    ...
    def _finish_workspace_change(self):
        ...
        Replaces the legacy `time.sleep(1.5)` on the GUI thread —
        the old behaviour froze the window for 1.5 s after every
        workspace change. The refresh and the show() are now split
        so the explorer rebuilds before the splash hides.
        """
        view = self._app_view
        if view is None:
            return
