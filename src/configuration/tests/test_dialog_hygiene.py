...
1. constructs a bare ``QMessageBox()`` / ``QErrorMessage()`` with no parent --
   these become top-level windows with no owner and get stacked *behind* the
   main window on X11/Wayland/Windows (the "app looks frozen" bug);
...
def _violations_in(path):
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Call):
            fn = node.func
            if ((_is(fn, "QMessageBox") or _is(fn, "QErrorMessage"))
