...
- **M5** — the four Schematic / Library converters (`pspiceToKicad`, `ltspiceToKicad`, `libConverter`, `LtspiceLibConverter`): the top-of-`convert` `os.path.getsize` now reads inside `try/except OSError` → a "File not found" dialog and early return, so a path the user typed or a source a sync client removed between the file-dialog pick and now no longer raises FileNotFoundError on the GUI thread. In the two schematic converters the post-success `shutil.copytree` into the workspace gets its own `try/except OSError` → a *warning* that reports the copy failure (locked / read-only / vanished target) while keeping the just-converted output, instead of the parser's success being lost to a raw excepthook. (Race-proof by reading the size rather than an `isfile` preflight, which is itself a TOCTOU.)
- **M10** — `Validation.validateSub` drops the `os.path.exists`-then-`open` TOCTOU for a direct `with open(...)` guarded by `except OSError` → a missing / unreadable / vanishing `.sub` returns the existing `"DIREC"` code; `validateSubcir`'s `os.stat` + `open` + read loop are wrapped in one `try/except OSError` → `False`. The `.sub` that disappears between the caller's exists-check and the read (fuzz cases C/D) no longer crashes.
- **H6** — `ProjectExplorer.renameProject` revert loop: every `os.rename` in the revert (per-file **and** the folder-name revert) is now individually guarded; failures collect in `stranded` and the directory is only folded back once its contents are consistent. A revert that hits the same Windows file-lock that broke the forward pass no longer escapes to the excepthook — the user gets a clear "mixed state — close KiCad and rename back manually" dialog naming the directory, instead of a half-renamed, `resolve_stem`-breaking project.
- **R3-13a** — the dead `Model.add_hex_btn` (never called anywhere; wired `self.addbtn.clicked.connect(self.addHex)` to an `addHex` that lives only on `Microcontroller`) is deleted — the latent AttributeError landmine is gone.

**Scope note:** the deeper "move user models / subcircuits under `~/.esim`" relocation (systemic note 2) stays deferred as in batch 2 — this batch only converts the crash into a dialog. The `renameProject` "refuse while `proc_dict[project]` has live children" preflight (the audit's *better* half of H6) is left out deliberately: it only catches eSim-launched editors (a manually-opened KiCad still locks the file), so the guarded revert — complete for every lock source — is the load-bearing fix. Converter subprocess scripts under `converter/**` keep their own module-level behavior (run only as standalone scripts, R2-8) — out of scope.
...
### Batch 8 — thread-safe reporting + dialog reentrancy (M9, M12) — DONE
Theme: systemic note 1 — *every dialog/widget touch must be born on the GUI thread*; "B1/B2/M9 are the same disease, one queued-signal error reporter fixes the class." That reporter now exists: a tiny GUI-thread `QObject` created in `Appconfig.attach_gui_reporter()` (called once from `Application` startup, on the GUI thread), backing both fixes. Verified by a new `audit_harness/verify_batch8.py` (13/13 offscreen), configuration pytest (18 passed), the `verify_batch4.py` + `smoke_no_qsci.py` regression harnesses (PASS), and ruff F821+bugbear clean on both touched files.

- **M9** — `Appconfig.print_info/warning/error` no longer touch the console `QTextEdit` / `QStatusBar` inline. They funnel through `_dispatch`, which emits the reporter's `note`/`status` signals (**AutoConnection**): a same-thread call still delivers directly and in order (byte-for-byte the old behaviour, so the GUI-thread hot path is unchanged), while a call from any worker thread is **queued onto the GUI thread** — so a future `BackgroundJob` fn calling `print_*` can no longer corrupt Qt state natively. Before the GUI attaches — and in headless/test runs it never does — there is no reporter, so `_dispatch` writes the plain-list sink directly (unchanged pre-GUI path). A reporter whose C++ side was torn down (a stale reporter left between test `QApplication`s) is caught (`RuntimeError`) and cleared, falling back to the direct sink instead of crashing the caller. `_append_note` also gained a `RuntimeError` guard for a destroyed `QTextEdit` at teardown.
- **M12** — the excepthook stops `exec()`ing its modal dialog inline. It posts the dialog through the **same** reporter via a dedicated `deferred` signal connected with **`QueuedConnection`**, so the box always opens on the next GUI event-loop turn: a raise inside a paint/close/teardown handler can no longer re-enter the event loop it is already running in (the `seen_sites` dedupe only ever stopped an infinite *storm*, not the first re-entrancy). En route this **repairs B1's marshalling, which was silently dead**: PyQt6's `QTimer.singleShot` has **no** `(msec, context, slot)` overload, so batch 1's `singleShot(0, app, show)` raised `TypeError` into the surrounding `except Exception: pass` on every worker-thread exception — the off-GUI-thread error dialog never actually appeared. A GUI-thread `QObject` signal is the only API that crosses threads correctly. Fallback for a crash so early the reporter does not exist yet: `singleShot(0, show)` (no context) — deferred on this thread's loop, which fires only on the GUI thread (the sole loop that early); a pre-reporter worker crash is already captured in `error.log`.

**Scope note:** the reporter is created only when the GUI attaches, so `import Appconfig` stays Qt-free for the many non-GUI importers (the `_GuiReporter` class is defined lazily inside `attach_gui_reporter`). The `note`/`status` signals keep AutoConnection deliberately — routing them through a queued connection would reorder console output relative to same-thread callers and add a needless event-loop hop on the GUI hot path; only the *dialog* needs the always-deferred QueuedConnection. Remaining "thread → dialog" siblings are already closed (B1 corrected here, B2 in batch 1).

**Numbering note:** like batches 6 and 7 this lands as an independent sibling off batch 4 (`b93c97b3`), not stacked — M9/M12 touch only `Appconfig.print_*` and the `Application` excepthook, which no other open batch changes, so batch 8 rebases cleanly onto whatever merge order the earlier siblings settle into.
### Batch 9 — dead-restore / false-success cleanup (R2-2, R3-7, R3-13 b/c/d) — DONE
Theme: the two "looks fine, does nothing" families the read/tool audit left open — prev-values restores that never ran, and the converter that reported success on an empty netlist — plus the remaining contained R3-13 leftovers. All fixes are additive guards / handle hygiene; none change a working path. Verified by `fuzz_ngmo.py` (K/L **false success → "no circuit elements to convert" error**, valid RC still succeeds, **0/12 raised**), `fuzz_ktn.py` (regression gate, still **0/10 raised**), and `kicadtoNgspice` + `ngspicetoModelica` + `codeEditor` pytest (**58 passed**); ruff F/B/F821 clean on all touched files. R3-13a (delete the dead `Model.add_hex_btn`) is **not** here — it already landed in Batch 6 (`077d30e0`); this batch deliberately leaves that method untouched to avoid a duplicate deletion.

- **R2-2** — `Microcontroller.py`: the prev-values restore was pure dead code. The parsed `<microcontroller>` node was stored to `self.root` while both restore loops (`:162`, `:226`) iterated a **never-defined local** `root` → a `NameError` swallowed by "Passes previous values" on every field, so the tab never restored a saved value and burned one caught exception per widget. Fixed to mirror the Model tab exactly: bind a local `root = None`, assign the parsed node to that local, and iterate `root if root is not None else []`. The restore now actually runs (identical logic to the proven-working Model tab); a missing/empty XML skips cleanly with no exception.
- **R3-7** — `ngspicetoModelica/ModelicaUI.callConverter`: an empty or comment-only `*.cir.out` parsed all the way to a bare skeleton `.mo` (no components) and then showed *"Ngspice netlist successfully converted…"* — false success plus a junk file. Added a guard right after `separateNetlistInfo`: when `schematicInfo` is empty (no device / source / subckt-instance line — the only content that makes a circuit) it shows a clear "netlist has no circuit elements to convert" error and **returns before any `.mo` is written**. `fuzz_ngmo` K/L now report the error; A–J unchanged.
- **R3-13 (b)** — `codeEditor/CodeEditor.reload`: `_load` → `_read_bytes` opens the file unguarded, so an external delete/rename between the watcher event and the user clicking Reload raised `FileNotFoundError` through the reload path. Wrapped the `_load()` call in `try/except OSError` → keep the current buffer instead of crashing.
- **R3-13 (c)** — `kicadtoNgspice/DeviceModel.GenerateSOCbutton`: the source-netlist and `.parsed.v` handles were opened raw and never closed (flushed only at GC). Both now use `with` — the netlist is read + closed immediately, and the generated Verilog is written under a context manager at the end.
- **R3-13 (d)** — `Source.py` / `Model.py` / `Microcontroller.py`: pre-bind `root = None` before the prev-values `try` so the restore name is **always** defined, retiring the latent unbound-`root` crash shape (the fuzz-proven R3-4-B pattern) even if future code ever reads `root` outside the swallowing `try`. Model's two restore loops also gained the explicit `if root is not None` guard (Source's six stay inside their existing `try/except`, which the pre-bind already makes unbound-proof). Analysis.py's `root[1][4]` restore was already repaired in Batch 3 (R3-9) and is untouched here.

**Scope note:** Batch 9 is a **sibling off `b93c97b3`** (batch 4 head), like batches 5–8 — not stacked on them. It shares no files with batches 6/8 except `Model.py`, where Batch 6 deletes `add_hex_btn` (end of file) and Batch 9 adds the `root` guards (top of `__init__` + the two restore loops) — disjoint hunks, clean 3-way merge. Batch 9 does **not** carry batch 5's test-infra changes. Remaining open after this batch: L2–L6/L9, R3-3 proof, and the VM-level environment matrix.
### Batch 10 — the LOW tail (L2, L3, L4, L5, L6, L9; clears R2-7) — DONE
Theme: the remaining edge-case LOW findings — cosmetic-but-noisy Qt warnings, silent asset/temp-file loss, a shutdown-stall backstop, a phantom-file launch, a GUI-thread subprocess freeze, and a cloud-sync refresh storm. Every fix is additive/defensive: no working path changes behaviour, each degrades to "nothing happens / clear dialog" instead of a warning storm, a phantom launch, or a frozen window. Verified by a new `audit_harness/verify_batch10.py` (**6/6 PASS**, offscreen), plus `ngspiceSimulation` pytest (**67 passed**), `test_plot_window_theme` + `projManagement` pytest (**37 passed**), and `frontEnd` `test_workspace_show` + `test_app_teardown` (**5 passed**). Ruff F/B/F821 introduces **no** new finding on any touched file and *removes* two pre-existing ones (see scope note).

- **L2** — `frontEnd/Application.py` splash: a missing/unreadable `splash_screen_esim.png` gives a null (0x0) `QPixmap`; the old code fed it straight through `scaledToWidth` → rounded-mask `QPainter`, spamming `QPainter::begin: Paint device returned engine == 0` and drawing an invisible splash. Now an `if splash_pix.isNull():` guard short-circuits to `splash = None` and skips the entire mask/paint/`FadingSplash` construction; the one inline `splash.close()` in the workspace-picker branch is `is not None`-guarded (every other consumer — `Workspace._finish_workspace_change` — already was). Startup is byte-for-byte identical otherwise. `verify_batch10` proves the unguarded chain warns and the guard is silent.
- **L3** — `ngspiceSimulation/plot_window._spin_arrow_icon`: the spin-box chevron PNGs are cached in the shared temp dir, and the cache check was a bare `os.path.exists`. A temp cleaner deleting/truncating the file between two `apply_theme` calls, or a silently-failing `QPixmap.save`, left the QSS `image: url(...)` pointing at a missing/zero-byte file (arrows vanish). Cache validity is now `exists AND getsize > 0` (wrapped in `try/except OSError`), so a missing **or** empty file re-renders; a failed `save` is tolerated (a missing glyph, no crash) and self-heals on the next `apply_theme` because the empty/absent file fails the stronger check. `verify_batch10` truncates, deletes, and forces a save failure.
- **L4** — `projManagement/Worker.WorkerThread.__del__`: the unbounded `self.wait()` during GC could stall process exit if a still-live child held the thread alive at interpreter shutdown. Capped to `self.wait(2000)`; the retention list (`Appconfig.worker_threads`) already keeps live workers off the GC path, so in practice `__del__` fires on a finished thread and returns at once — the cap is purely a shutdown-stall backstop. `verify_batch10` asserts `wait(2000)` is the exact call.
- **L5** — `projManagement/Kicad.openSchematic`: `main_schematic` returns a best-guess `<stem>.kicad_sch` even when nothing exists on disk, so eeschema was launched on a phantom path (silent blank/new schematic, or eeschema's own confusing error). Added `if not os.path.exists(schematic_file):` → a clear `Dialogs.critical` + `print_warning` + `return` **before** the `WorkerThread` is built. `verify_batch10`: missing file → exactly 1 dialog, 0 eeschema launches.
- **L6** — `converter/pspiceToKicad.PspiceConverter.convert`: the parser ran via a synchronous `subprocess.run` on the GUI thread, so a large schematic froze eSim ("Not Responding") for the whole parse. Moved onto a `maker.hdl.jobs.BackgroundJob` (the same pattern the KiCad netlister uses in `projManagement/Kicad.py`): a static `_run_parser` does the blocking `subprocess.run` on the worker thread and returns `(returncode, stdout, stderr)` (never raising on a non-zero exit), and the GUI-thread slots `_on_convert_done` / `_on_convert_failed` route success (workspace copy + dialog), a non-zero exit (failure dialog, same detail surface as before), or an OSError launching the interpreter (previously an unguarded GUI-thread raise). A `_convert_job.isRunning()` guard refuses a second Schematic click while one runs. `verify_batch10` proves the job is scheduled and returns at once, the double-run is refused, and both slots fire the right dialog.
